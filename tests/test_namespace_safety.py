...
print('== tuple unpacking + preparsed sage constructs ==')
check('tuple unpacking of names', stmt_eligible('(a, b) = f()') == 'assign')
check('tuple unpacking (no parens)', stmt_eligible('a, b = f()') == 'assign')
check('preparsed _first_ngens call', rhs_safe('R._first_ngens(1)'))
check('preparsed generator unpack', stmt_eligible('(x,) = R._first_ngens(1)') == 'assign')
check('rejected: tuple with unsafe RHS', stmt_eligible('(a, b) = eval("1")') is None)
check('rejected: tuple with attribute elt', stmt_eligible('(a, obj.x) = f()') is None)
check('rejected: starred in tuple target', stmt_eligible('(a, *b) = f()') is None)
check('rejected: other underscore attr call', not rhs_safe('R._secret(1)'))
check('rejected: dunder attr still blocked', not rhs_safe('obj.__dict__'))
